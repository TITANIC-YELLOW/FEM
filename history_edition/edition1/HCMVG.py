...
def IntInTetra(func) -> float:
    ans = 0.
    for i in range(27):
        x, y, z, const1, const2 = TetraIntPoints[i]
        ans += func(x, y, z) * const1 * const2
...
def k_tetra(CoordinateArray:np.ndarray, conductivity:list) -> np.ndarray:
    '''
    ...
    k1 = conductivity[0] * np.array([[b1*b1, b1*b2, b1*b3, b1*b4],
                                            [b1*b2, b2*b2, b2*b3, b2*b4],
                                            [b1*b3, b2*b3, b3*b3, b3*b4],
                                            [b1*b4, b2*b4, b3*b4, b4*b4]])
    k2 = conductivity[1] * np.array([[c1*c1, c1*c2, c1*c3, c1*c4],
                                            [c1*c2, c2*c2, c2*c3, c2*c4],
                                            [c1*c3, c2*c3, c3*c3, c3*c4],
                                            [c1*c4, c2*c4, c3*c4, c4*c4]])
    k3 = conductivity[2] * np.array([[d1*d1, d1*d2, d1*d3, d1*d4],
...
def k_prism(CoordinateArray:np.ndarray, conductivity:list) -> np.ndarray:
    '''
    ...
    for j in range(8):
        ...
        for row in range(6):
            for col in range(row+1):
                ...
                for i in range(3):
                    ans = conductivity[i] * det_J * D[i, row] * D[i, col] * const
                    k[row, col] += ans
                    k[col, row] += ans
...
def k_prism27(CoordinateArray:np.ndarray, conductivity:list) -> np.ndarray:
    '''
    ...
    for j in range(27):
        ...
        for row in range(6):
            for col in range(row+1):
                ...
                for i in range(3):
                    ans = conductivity[i] * det_J * D[i, row] * D[i, col] * const *const2
                    k[row, col] += ans
                    k[col, row] += ans
...
def k_pyramid(CoordinateArray:np.ndarray, conductivity:list) -> np.ndarray:
    '''
    ...
    for row in range(5):
        for col in range(row + 1):
            ...
            def func(x,y,z):
                ...
                res = 0.
                for i in range(3):
                    res += det_J * D[i, row] * D[i, col] * conductivity[i]
...
def k_hexa(CoordinateArray:np.ndarray, conductivity:list) -> np.ndarray:
    ...
    for j in range(8):
        ...
        for row in range(8):
            for col in range(row+1):
                ...
                for i in range(3):
                    ans = conductivity[i] * det_J * D[i, row] * D[i, col]
                    k[row, col] += ans
                    k[col, row] += ans
    for idx in range(8):
        k[idx,idx] *= 0.5
...
def S_2d_quad(CoordinateArray:np.ndarray) -> float:
    def func(x, y):
...
def k_quad(CoordinateArray:np.ndarray, conductivity:list) -> np.ndarray:
    k = np.zeros((4,4))
...
def m_tri(CoordinateArray:np.ndarray, p:float, c:float) -> np.ndarray:
    ...
    m = p * c * S_2d_tri(CoordinateArray) / 12 * np.array([[2, 1, 1],
                                                           [1, 2, 1],
                                                           [1, 1, 2]])
...
def k_tri(CoordinateArray:np.ndarray, conductivity:list) -> np.ndarray:
    con_x, con_y = conductivity
...
def m_axisymmetric_tri(CoordinateArray:np.ndarray, p:float, c:float) -> np.ndarray:
    '''
    :param CoordinateArray: (3,2)
    :param p: density
    :param c: specific heat
    :return: mass matrix
    '''
    r_c = np.mean(CoordinateArray[:, 1])
    m = m_tri(CoordinateArray, p, c) * np.pi * r_c * 2
    return m


def k_axisymmetric_tri(CoordinateArray:np.ndarray, conductivity:list) -> np.ndarray:
    '''
    :param CoordinateArray: (3,2)
    ...
    '''
    r_c = np.mean(CoordinateArray[:, 1])
    return k_tri(CoordinateArray, conductivity) * np.pi * 2 * r_c


def m_axisymmetric_quad(CoordinateArray:np.ndarray, p:float, c:float) -> np.ndarray:
    r_c = np.mean(CoordinateArray[:, 1])
    m = m_quad(CoordinateArray, p, c) * np.pi * r_c * 2
    return m


def k_axisymmetric_quad(CoordinateArray:np.ndarray, conductivity:list) -> np.ndarray:

    r_c = np.mean(CoordinateArray[:, 1])
    return k_quad(CoordinateArray, conductivity) * np.pi * 2 * r_c


def f2_axisymmetric_line(CoordinateArray:np.ndarray, Q:float) -> np.ndarray:
    r1 = CoordinateArray[0,1]
    r2 = CoordinateArray[1,1]
    f = np.array([2*r1+r2, r1+2*r2])
    return f * Q * length_of_line(CoordinateArray) * np.pi / 3
...
def f3_axisymmetric_line(CoordinateArray:np.ndarray, h:float, Tf:float) -> np.ndarray:
    ...
    r2 = CoordinateArray[1,1]
    f = np.array([2*r1+r2, r1+2*r2])
    return f * h * Tf * length_of_line(CoordinateArray) * np.pi / 3
...
def edge_axisymmetric_line(CoordinateArray:np.ndarray, h:float) -> np.ndarray:
    ...
    r1 = CoordinateArray[0, 1]
    r2 = CoordinateArray[1, 1]
    edge = length_of_line(CoordinateArray) * h * np.pi / 6 \
           * np.array([[3*r1+r2,r1+r2],[r1+r2,r1+3*r2]])

    return edge


def version() -> str:
    return 'The version number: 20240412'
