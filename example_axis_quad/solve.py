...
from HCMVG import m_axisymmetric_quad, k_axisymmetric_quad, \
    f3_axisymmetric_line, edge_axisymmetric_line
...
points, cells, faces = read_quad_dat.getdata('axis_quad.dat')
...
def create_m(i:int) -> np.ndarray:
    ...
    coor = np.zeros((4, 2))     #   获取8节点坐标数组

    for j in range(4):
        coor[j] = points[int(cells[i, j])]

    m_e = m_axisymmetric_quad(coor,p,c)
...
def create_k(i:int) -> np.ndarray:
    ...
    k_e = k_axisymmetric_quad(coor, lamda)
...
def create_edge(i:int) -> np.ndarray:
    ...
    edge_e = edge_axisymmetric_line(coor,h)
...
def create_f(i:int) -> np.ndarray:
    ...
    f_e = f3_axisymmetric_line(coor, h, 1000)
